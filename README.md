...
- 专家模式：选择一个专家时沿用单 Agent 专家 Prompt；选择两个及以上专家并启用真实 AgentScope 后，自动进入专家团模式。系统会并行创建通用 Expert Worker，每个 Worker 只接收自己的专家 Prompt，主 Agent 只汇总 Worker 结果。
- 专家团边界：Worker 首版只负责独立分析，不直接修改文件或调用外部工具；主 Agent 继续负责工作区文件、Skill、MCP、审批和最终交付。每个 Worker 输出最多 6000 字符，汇总进入主 Agent 的结果最多 24000 字符，专家团单轮最多执行 600 秒。
